...
'Frase examen: con dos grupos la “variabilidad” se reduce un 41.39456 %'
...
abline(h=10,col='red')
'Estados más similares: Fleming (AUS) - Dimitrova (BUL)'
...
'Frase examen: con dos grupos la “variabilidad” se reduce un 72.61867 %'
...
1 - (CA1$tot.withinss)/(CA1$totss)
'Frase examen: con dos grupos la “variabilidad” se reduce un 60.73306 %'
...
'Hay un error porque hay una trampa en el dataset.'
...
# Invertimos las carreras para que un tiempo bajo sea un valor alto (bueno)
# Una forma fácil es restar el tiempo al tiempo máximo registrado, 
# o simplemente ponerles un signo negativo.
d$hurdles <- max(d$hurdles) - d$hurdles
d$run200m <- max(d$run200m) - d$run200m
d$run800m <- max(d$run800m) - d$run800m

# Opcional: Quitar la columna 'score' (columna 8)
# Como la puntuación final es un resumen de todo lo anterior, suele quitarse 
# para que el cluster se base solo en el esfuerzo físico.
d <- d[, 1:7]
